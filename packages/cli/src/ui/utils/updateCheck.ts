...
import { getPackageJson, debugLogger } from '@google/gemini-actus-core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of Gemini Actus is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `Gemini Actus update available! ${currentVersion} → ${latestUpdate}`;
