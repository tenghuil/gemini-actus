...
import { isGitRepository, debugLogger } from '@google/gemini-actus-core';
...
describe('getInstallationInfo', () => {
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@google/gemini-actus/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g @google/gemini-actus@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@google/gemini-actus/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add @google/gemini-actus@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @google/gemini-actus@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should NOT detect Homebrew if gemini-cli is installed in brew but running from npm location', () => {
    ...
    const cliPath =
      '/usr/local/lib/node_modules/@google/gemini-actus/dist/index.js';
    process.argv[1] = cliPath;
